...
export default async function AdminPage() {
            ...
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 px-4">
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl p-10 text-center max-w-md">
                    ...
                    <p className="text-slate-400 text-sm mb-6">
                        このページは全体管理者（global_admin）のみアクセスできます。
                    </p>
                    <Link
                        href="/dashboard"
    ...
    const { data: allRequests } = await admin
        .from("ks_requests")
        .select("club_id, total_amount, status")
    ...
    const { data: pendingRequests } = await admin
        .from("ks_requests")
        .select("id, date, applicant_name, category, reason, total_amount, club_id, receipt_url")
        .eq("status", "submitted")
        .is("deleted_at", null)
        .order("created_at", { ascending: false });

    // club_id → club name のマッピング
